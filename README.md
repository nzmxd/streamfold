...
- 跨账号内容中心提供 FTS5 全文检索、可保存的组合筛选、账号级排序记忆、部分同步内容筛选、本地收藏、批量标签/链接复制、官方原帖入口和筛选范围 JSON/CSV 导出。
...
- 本地日志中心统一保存应用、同步、插件和后台任务的完整原因链；插件异常包含版本、入口、执行方法、函数、源码位置、调用链和有界脱敏响应详情，只移除账号凭证、认证头和 Session。
