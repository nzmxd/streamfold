...
- 统一任务中心汇总账号同步和插件运行，支持状态/平台/账号/来源筛选、批次进度、排队取消、失败重试和重启恢复。
...
- Manifest v2 开放插件宿主、QuickJS Utility Process 沙箱、细粒度授权、签名目录、本地开发插件、事件 Outbox、计划队列和官方 Webhook。
...
- 小红书、知乎是可信内置实现；签名第三方插件在独立 Utility Process 的 QuickJS 上下文运行，只能调用获授权的宿主代理。
- SQLite schema 当前为 v13；五项通用内容指标保留兼容列，平台扩展内容指标和账号周期指标使用定义表与快照值表保存完整变化历史。排队任务与批次可在重启后恢复，运行中任务会标记为中断，内容和指标仍按原子事务提交。
...
- 插件事件和计划均默认关闭，需要用户选择账号/分组并授权；账号自动同步计划仍未开放，当前批量同步由用户主动发起。
- 远程插件发现的目录 URL 与根公钥会编译进发布构建；0.6.3 未配置远程目录时会明确禁用“发现”，内置平台、签名 Webhook 和开发者模式仍可使用。
