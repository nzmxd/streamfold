...
- SQLite schema 当前为 v12；五项通用指标保留兼容列，平台扩展指标使用定义表与快照值表保存完整变化历史。排队任务与批次可在重启后恢复，运行中任务会标记为中断，内容和指标仍按原子事务提交。
...
- 远程插件发现的目录 URL 与根公钥会编译进发布构建；0.6.1 未配置远程目录时会明确禁用“发现”，内置平台、签名 Webhook 和开发者模式仍可使用。
