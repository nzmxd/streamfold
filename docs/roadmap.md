...
`0.7.14` 已经建立后续版本所需的主要基础设施；`0.7.10` 到 `0.7.14` 进一步加入部分成功同步状态、覆盖信息、清单驱动的通用后台捕获、可复用的内容筛选工作流、可靠退出清理和大型平台响应兼容：

| 能力 | 当前状态 | 后续关注点 |
|---|---|---|
| 账号与 Session | 多平台、多账号、独立 Partition、官方入口登录、账号级任务互斥 | 登录失效长时间运行验收、窗口位置恢复 |
| 平台同步 | 小红书、知乎和 X 本人资料、内容、指标 | 异常场景、更多分页、微博与抖音逐平台验收 |
| 内容与统计 | FTS5 内容中心、保存筛选视图、账号排序记忆、部分同步内容筛选、批量链接复制、筛选导出、观察历史、可靠指标语义与跨账号分析 | 封面缓存、更多导出格式和真实大库回归 |
| 任务 | 统一读模型、账号/分组批量队列、任务中心、失败处置、托盘摘要、取消/重试和恢复 | 真实多账号长时间运行、更多自动同步策略 |
| 自动计划 | 插件间隔/每天/每周/每月计划、启停、删除、错过周期合并与熔断 | 编辑范围、预览、重复检测、执行历史增强 |
| 插件系统 | Manifest v2、QuickJS、授权、目录、更新、Webhook、SDK | 正式目录仓库、SDK 发布、诊断和生态验收 |
| 数据安全 | SQLite 事务、加密备份、IPC 校验、沙箱限制 | 迁移回滚演练、代码签名、安全专项审计 |
| 发布 | 三平台构建、公开 GitHub Release 和 Windows/Linux 更新资产 | 正式签名、公证与真实跨版本在线更新实测 |
...
| `0.7.0` | 内容与数据分析 | FTS5、指标增量、内容趋势、跨账号比较、任务处置与日历计划 | 已实现，待正式构件与真实数据验收 |
| `0.7.1` | 稳定性与可观测性增量 | 应用日志、错误处置、分析与界面回归完善 | `0.7.0` 功能基线 |
| `0.7.2` | 主题个性化 | 自定义主题颜色与相关界面回归 | `0.7.1` 稳定性基线 |
| `0.7.3` | X 网页会话接入 | 签名 X 适配器、稳定身份绑定、资料与本人帖子同步 | `0.7.2` 插件与 Session 基础 |
| `0.7.4` | X 账号创建修复 | 统一单字符平台 ID 的清单、IPC 与筛选合同 | `0.7.3` X 接入 |
| `0.7.5` | X 内置行为与登录兼容 | 默认启用授权、配置与计划、标准 Chromium 登录身份 | `0.7.4` X 账号创建修复 |
| `0.7.6` | X 身份核验可靠性 | 有界响应捕获、真实登录身份复核和固定诊断码 | `0.7.5` X 默认行为与登录兼容 |
| `0.7.7` | X 首响应等待修复 | 等待延迟身份响应、阶段化安全诊断和真实会话回归 | `0.7.6` X 身份核验可靠性 |
| `0.7.8` | 诊断与品牌更新 | 完整错误详情、敏感信息脱敏、海蓝主题与跨平台图标 | `0.7.7` 稳定基线 |
| `0.7.9` | X 后台身份发现 | 工作区常驻监听、投影缓存、自动绑定提示与鲜活复核 | `0.7.8` 诊断与品牌基线 |
| `0.7.10` | 同步完整性与内容工作流 | 部分成功状态、覆盖统计、X 分页兼容、默认排序与复制原帖链接 | `0.7.9` 后台身份发现 |
| `0.7.11` | 通用后台捕获 | 插件清单字段白名单、统一 supervisor、健康状态、自动重连与跨平台绑定提示 | `0.7.10` 同步完整性基线 |
| `0.7.12` | 内容工作流增强 | 保存筛选视图、账号排序记忆、部分同步内容筛选和批量复制链接 | `0.7.11` 通用后台捕获基线 |
| `0.7.13` | 退出稳定性修复 | 窗口销毁后的 IPC 监听清理与重复注销回归覆盖 | `0.7.12` 内容工作流基线 |
| `0.7.14` | X 大型响应兼容 | 隔离平台 JSON 与插件消息结构预算，恢复合法大型 UserTweets 响应同步 | `0.7.13` 退出稳定性基线 |
...
- 操作页错误使用面向操作的说明并保留稳定错误代码；日志中心同时展示插件源码位置、原因链和有界脱敏响应详情，Electron 内部对象、请求认证头和账号凭证不进入日志。
...
本版本已经交付可追溯的搜索、增量和对比，不引入不可解释的“账号总分”。以下条目同时作为实现边界与回归标准。
...
- SQLite v16 增加内容收藏、标签、每次成功同步的观察记录、按贡献点与包哈希固化的指标语义，以及可重建的 FTS5 trigram 索引；v14/v15 分别保存计划 cadence 和任务失败处置。
- 内容中心默认每页 50 条、最多 100 条，支持标题/摘要/标签/备注搜索、组合筛选、相关度或指标排序、批量标签/收藏及筛选范围 CSV/JSON 导出。
- 分析中心提供概览、账号/平台/分组对比、24 小时/7 天/30 天生命周期和数据质量；区分 `cumulative`、`period_total`、`gauge`，并处理缺失、修订、待观察和不完整数据。
- 任务失败历史保持不可变，用户可手动标记已处理，后续同一主体成功会自动消解旧失败；插件计划增加间隔、每天、每周和每月模式，按本地时区执行并合并错过周期。
- 2026-07-15 Windows 最终发布验证使用 10 万条内容和 20 万条观察/快照：FTS 首屏中位 `1.54 ms`，全量分页 `3.67 s`，约 22.37 MB CSV 序列化 `0.29 s`，全量可靠分析摘要 `5.92 s`。结果只用于同环境回归比较。

### 6.7 `0.7.3` 增量交付

- X 作为默认关闭的官方签名 QuickJS 适配器分发，沿用账号隔离 Session、身份预览确认、同步前后复验和统一数据提交流程。
- 只捕获 X 官方页面自身发出的固定 HTTPS JSON/GraphQL `GET` 响应，不使用开发者 API、Token、Cookie 导入、DOM 或签名逆向。
- 使用稳定账号 ID 绑定身份，同步本人资料、原创帖与引用帖，过滤回复、纯转推和其他作者内容；分页、重复数据或身份不一致时拒绝提交。
- 官方 Webhook 与 X 插件统一经过源码一致性、Ed25519 签名、固定哈希、安装目录资源和 QuickJS 运行 Smoke 校验。

### 6.8 `0.7.4` 增量交付

- 修正账号创建与平台筛选的 IPC 校验，使清单允许的单字符平台 ID `x` 可以进入现有账号、Session 和同步流程；字符集与 128 字符上限保持不变。

### 6.9 `0.7.5` 增量交付

- X 1.1.0 作为固定信任的默认平台能力初始化：首次或旧版未授权状态会启用包与贡献点、建立受托授权，并为后续 X 账号自动扩展范围；用户手动关闭或修改授权后不再反转。
- 增加手动采集间隔配置、账号或分组运行计划和试运行；沙箱同步实际读取配置值，撤销 `scheduler.run` 会立即停用既有计划。
- X 与知乎使用相同的登录兼容策略，只去除 Electron 与应用包装产品标识，保留真实操作系统和 Chromium 版本；平台临时限制、验证与风控仍由用户在官方页面处理。

### 6.10 `0.7.6` 增量交付

- 修复 X 首页并发产生重复身份响应时的捕获竞态：候选按请求顺序有界串行读取，严格限制返回数量、正文大小、总字节和请求跟踪规模，并为初始化、导航、正文与清理设置宿主截止时间。
- X 1.1.1 每次先用 `account/settings` 与 `UserByScreenName` 返回实际当前登录身份，兼容 `data.user` 和操作专用结果根；仅在身份仍匹配时再用 `UserByRestId` 复核稳定账号 ID。
- 登录账号切换会交由宿主标记为身份不一致；其他沙箱身份失败使用固定 `PLUGIN_ADAPTER_IDENTITY_FAILED` 诊断码和脱敏提示，不暴露 Cookie、请求头、响应正文或插件内部异常。

### 6.11 `0.7.7` 增量交付

- 修复声明式 JSON 捕获在首个匹配响应到达前因 800 毫秒静默窗口提前返回空数组的问题；只有成功捕获响应后才允许静默收敛，零响应继续等待受控截止时间。
- X 1.1.2 将账号设置缺失、当前资料缺失、响应结构变化和稳定 ID 复核失败报告为单字段枚举；宿主仅为官方 X 贡献点映射固定错误码和操作提示，未知或附带字段的结果继续安全降级。
- X 1.1.3 移除不与当前会话绑定且会受页面缓存、重定向和无关同名请求影响的 `UserByRestId` 反查；确认与同步继续重复 `account/settings` → `UserByScreenName`，并由宿主严格比较稳定 ID，账号切换仍会停止同步。交互式账号浏览器优先打开 X 首页，避免已登录 Session 被登录入口重复拉起通行密钥流程。
- 使用现有隔离 X Session 完成 `account/settings`、`UserByScreenName`、Utility Process 和 QuickJS 的端到端回归；验证过程不读取或输出 Cookie、令牌、请求头和账号值。

### 6.12 `0.7.8` 增量交付

- 日志中心保留异常堆栈、原因链、聚合错误、HTTP 状态和有界结构化属性；同步、插件、第三方适配器与后台队列失败均进入同一诊断链路，不再用摘要替代错误详情。
- token、Cookie、Authorization、密码、JWT、URL 凭据和私钥在写入、历史迁移、列表、复制与导出边界统一脱敏；历史日志使用单实例保护下的原子清洗，并缓存已清洗结果以避免大日志反复阻塞主进程。
- 默认主题色更新为海蓝，应用矢量母版、运行时 PNG、Windows ICO、macOS ICNS、Linux 与托盘资源同步更新，并保留用户主动选择的自定义主题色。

### 6.13 `0.7.9` 增量交付

- X 身份发现改为账号工作区级常驻监听：后台插件调用不再等待默认捕获超时，命中设置与资料响应后自动推进，未绑定账号只提示一次。
- 常驻缓存按账号 Session、官方插件版本、贡献点、规则和参数隔离，2 分钟失效；只保存主进程投影后的身份必要字段，不保存请求头、Cookie、Authorization、完整 URL、查询变量或原始响应。
- 后台发现返回 `capture_pending` 而不是错误；用户确认和同步仍停用发现缓存并执行鲜活响应核验，避免账号切换后复用旧身份。
- X 1.1.4 将 `recent_20` / `recent_100` 的时间线响应上限提高到 5 / 20；有界监听停止且仍有 Bottom cursor 时提交已验证本人内容，并记录实际响应数、上限和范围警告，重复游标、冲突重复和结构错误继续拒绝整次同步。
- 所有 QuickJS action、event、schedule 和 platform contribution 的协议、权限、资源、进程崩溃与执行异常均保留插件版本、入口、执行方法、调用函数、源码上下文和原因链；网络与平台宿主异常同时保留有界脱敏响应详情。
...
- 提供诊断导出：包含版本、状态、错误代码、插件源码定位和有界脱敏响应，不包含 Secret、Cookie、Authorization、Session、请求认证头或无限制业务数据。
...
- 插件诊断不泄露 Secret、Cookie、Authorization、Session、请求认证头和未授权账号数据；授权范围内的响应只保留有界、脱敏片段。
...
- 操作成功提示简短；前台错误提示包含下一步，完整堆栈、IPC、插件源码位置和有界脱敏响应集中显示在日志中心。
...
2. 增量分析、跨账号对比、任务失败处置与日历计划已经在 0.7.0 完成。
3. 下一步完成真实多账号、登录失效、风控和长时间运行验收。
...
- 运行计划编辑范围、执行预览、重复检测和立即试运行的剩余界面与合同。
