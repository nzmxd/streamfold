...
```json
{
  "backgroundCapture": {
    "captures": [
      {
        "captureId": "identity-settings",
        "responseFieldPaths": ["/screen_name"]
      },
      {
        "captureId": "identity-profile",
        "responseFieldPaths": [
          "/data/user/result/id",
          "/data/user/result/name",
          "/data/user/result/screen_name"
        ],
        "responseCorrelations": [
          {
            "routeParameter": "handle",
            "responseFieldPaths": ["/data/user/result/screen_name"],
    ...
    "identityDiscovery": {
      ...
      "captureIds": ["identity-settings", "identity-profile"]
    }
  }
}
```

统一 supervisor 只缓存 `responseFieldPaths` 白名单中的标量字段，并对字符串再次执行凭证脱敏；路径使用 RFC 6901 风格，`*` 只能出现在非末尾的数组项位置，凭证字段别名、整对象投影、终端通配和空投影都会被拒绝。捕获页面路由含有未被 `responsePath` 绑定的模板参数时，必须用 `responseCorrelations` 指定响应中回显该参数的无通配白名单字段；不匹配的同 operation 响应会被忽略，不能覆盖当前账号缓存。supervisor 同时维护监听、降级、重试和恢复状态，连接、页面加载或响应读取失败后使用声明的初始/最大间隔做指数退避。关闭账号浏览器窗口时，仍在监听的工作区会转入后台；断开账号、停用或更新插件、撤销授权、切换适配器及退出应用时才会停止。首次进入未绑定账号的官方页面会调用一次 `readIdentity` 以启动监听；此后 `on-navigation-and-capture` 在导航及捕获身份响应时继续发现，`on-capture` 只在捕获新响应时继续发现。后台缓存尚未捕获到必需响应时，`readIdentity` 应检查 `context.capturePolicy` 并返回 `{ "status": "capture_pending" }`，宿主会继续监听而不记录为失败；识别到身份后仍必须由用户确认绑定。
