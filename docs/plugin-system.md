...
2xx 表示成功；网络错误、408、425、429 和 5xx 按 1 分钟、5 分钟、15 分钟、1 小时、6 小时、24 小时退避，并取服务器合法 `Retry-After` 与本地退避的较大值。其他 4xx 直接进入失败记录，可从“运行记录”手动重试。日志只保存状态和清洗后的错误，不保存 Secret、完整响应正文或平台 Session。
