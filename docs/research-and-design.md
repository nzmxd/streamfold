...
作品管理接口提供完整的本人作品集合，数据分析接口提供当前分析范围内可见的指标。客户端按作品 ID 合并两类结果：作品管理结果作为内容基线，数据分析结果补充浏览、点赞、收藏、评论和分享等指标。没有返回的指标保留为空值。

### 2.4 其他平台结论

微博、抖音和知乎可以复用相同的账号浏览器与适配器合同，但不能仅凭开源项目存在就宣称支持。每个平台都需要完成：
...
- 主进程按平台清单校验 HTTPS 主机、导航、重定向、子框架和弹窗目标。
- 下载和站点权限默认拒绝。
- 平台页面没有主进程 IPC，也不能访问其他账号会话。

内置 Chromium 是普通 Electron/Chromium 运行环境，不实现代理轮换、Canvas/WebGL 修改、User-Agent 伪装或验证码处理。官方入口登录可以减少误入非官方页面的风险，但不能保证平台永远不触发重新验证。
...
2. **签名响应捕获**：对于必须由页面生成签名参数的接口，先连接 Chromium DevTools Protocol 的 `Network` 域，再打开固定业务路由，只读取匹配白名单路径的 Fetch/XHR JSON 响应正文。
...
头像是资料媒体，不交给 Renderer 直接下载。`userAvatar` 必须是小红书域名或 CDN 下的 HTTPS URL；主进程下载时不带账号凭据或 Referrer，对重定向逐次复验，仅接受 JPEG、PNG、WebP、GIF 或 AVIF，并同时校验 MIME、文件头、Content-Length、实际长度和 512 KiB 上限。缓存键由 SHA-256 内容哈希生成，界面只读取 `app://shell/media/avatars/...`。
...
2. 适配器同时读取 `user/info` 与 `personal_info`，先交叉校验两者远端 ID，再返回昵称和 ID 预览。
3. 用户确认这是自己的账号。
4. 适配器再次读取并交叉校验两组身份 API，结果一致后写入 `remote_id`；本地备注名未自定义时采用平台昵称。
...
| `contents` | 本人内容主记录，以账号和远端 ID 去重，包含类型、标题、链接、时间、标签和备注 |
...
- Electron + Vue 3 + TypeScript 桌面外壳和 SQLite v6。
- 多平台账号空间、分组、标签、备注、默认账号和批量管理。
- 每账号独立 Chromium 工作窗口及持久 Session Partition。
- 小红书双接口身份交叉校验、头像本地缓存、资料、账号指标、本人作品列表和作品指标同步。
...
1. 完成小红书真实多页大账号、限流、登录失效、账号切换和空数据的更多环境测试。
2. 增加定时同步、Retry-After 冷却和连续失败熔断。
3. 按相同合同接入微博、抖音和知乎，每个平台独立验收后启用。
4. 完成 Windows、macOS、Linux 安装包、签名、自动更新和 CI 发布。
5. 在后台 workspace lease、账号互斥和同平台单并发基础上实现串行批量同步队列。
