...
| 数据 | 接口 | 获取方式 |
|---|---|---|
| 身份与资料 | `GET /api/galaxy/creator/home/personal_info` | 在已登录创作中心页面同源请求固定接口 |
...
1. **同源固定请求**：浏览器已经位于允许的创作中心 HTTPS 页面时，用页面原生 `fetch` 请求白名单中的固定 `GET` 接口，携带该账号 Session 的登录语义。
...
1. 用户在该账号浏览器中完成平台登录。
2. 适配器读取身份 API，返回远端 ID 和昵称预览。
3. 用户确认这是自己的账号。
4. 适配器再次读取身份 API，结果一致后写入 `remote_id`。
...
当前 SQLite schema 版本为 v5。

| 表 | 作用 |
|---|---|
| `accounts` | 平台、本地别名、远端身份、连接/归属/同步状态、备注、标签、Session Partition 和同步范围 |
...
- Electron + Vue 3 + TypeScript 桌面外壳和 SQLite v5。
- 多平台账号空间、分组、标签、备注、默认账号和批量管理。
- 每账号独立 Chromium 工作窗口及持久 Session Partition。
- 小红书身份、资料、账号指标、本人作品列表和作品指标同步。
