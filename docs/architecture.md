...
| 数据库迁移 | `PRAGMA user_version`，当前 v10 | `src/main/storage/migrations.ts` |
...
插件宿主提供持久事件 Outbox 和统一计划队列；所有事件/定时计划默认关闭，由用户选择账号或分组后启用。主动单账号同步仍可直接触发，自动同步还需遵守平台最小一小时及适配器自身限频。
...
schema 通过 `PRAGMA user_version` 逐版迁移，当前为 v10。迁移在 `BEGIN IMMEDIATE` 事务中执行；数据库版本高于应用支持版本时拒绝打开。v9 增加账号适配器绑定、插件包/贡献点/授权/配置/计划/事件/投递/运行记录，并迁移小红书与知乎账号绑定；v10 删除已不可达的文件导入批次表，既有账号、内容和快照不受影响。
