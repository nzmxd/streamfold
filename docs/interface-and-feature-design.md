...
Social Vault 把账号登录、账号整理、本人内容同步和统计分析放在一个桌面应用中。界面遵循三条原则：
...
- 搜索范围包括本地别名、平台昵称、备注和标签。
- 可按全部账号、未分组、登录异常、已暂停和自定义分组筛选。
- 每行显示平台、头像或平台简称、本地别名、平台昵称和当前状态。
- 支持多选当前结果，批量加入或移出分组，以及批量暂停或恢复同步。
- 创建账号时选择平台、本地别名和初始同步范围；创建完成后立即给出“打开浏览器”的下一步。
...
| 页签 | 内容 |
|---|---|
| 总览 | 连接状态、身份状态、同步范围、最近同步、核心指标和使用步骤 |
| 浏览器 | 浏览器窗口状态、当前地址、打开或切换到独立窗口、核验身份和同步 |
| 内容数据 | 该账号最近内容、最新指标、采集时间和内容中心入口 |
| 设置与备注 | 本地别名、备注、标签、分组、同步范围、默认账号和暂停状态 |
...
| 维度 | 主要状态 | 用户操作 |
|---|---|---|
| 连接 | 等待登录、可用、已过期、不匹配、已断开 | 打开浏览器、重新登录或断开会话 |
| 身份 | 未确认、待确认、已核验 | 核对平台昵称与远端 ID，确认本人账号 |
| 同步 | 未开启、空闲、运行中、冷却、失败、平台暂不可用 | 调整范围、重试或等待冷却结束 |

只有登录可用、身份已核验、同步范围不是 `disabled` 且用户已开启同步时，才允许开始平台同步。
...
首次核验生成 5 分钟有效的确认令牌。用户确认后，应用再次读取身份 API；两次结果一致才绑定远端 ID。后续核验直接比较当前 API 身份和本地绑定身份。
...
当前同步由用户手动触发。按钮在不满足登录、身份或授权条件时禁用，并在附近显示可执行的恢复动作。
...
- 查看标题、账号、发布时间、采集时间和最新浏览、点赞、评论、分享、收藏指标。
- 查看相较上一个快照的指标变化。
- 进入原平台内容或数据详情页。
...
当前只有 `xiaohongshu-session-api` 可启用；微博、抖音和知乎作为计划中适配器展示，不能执行同步。
...
0.4.0 已实现账号管理、分组备注、独立浏览器、小红书身份核验与手动同步、内容与分析、插件状态、导出和加密备份恢复。

后续优先事项：

1. 完成小红书真实多页大账号、429 冷却、登录过期和账号切换的更多环境验收。
2. 增加可配置的定时同步、连续失败熔断和恢复提示。
3. 按相同 `session_api` 合同逐个平台接入微博、抖音和知乎；没有稳定 JSON 接口的平台保持不可用。
