...
插件 ID：`streamfold.x`，版本 1.1.4；随应用签名分发，在 QuickJS Utility Process 中运行，最低要求归页 0.7.9。首次安装或从旧版未授权状态升级时默认启用并建立受托授权，用户之后仍可关闭包、贡献点或收窄授权。手动同步间隔默认且最少为 5 分钟，可配置到 24 小时；自动计划最短 1 小时，建议周期 24 小时。
