...
插件 ID：`streamfold.x`，版本 1.1.5；随应用签名分发，在 QuickJS Utility Process 中运行，最低要求归页 0.7.9。首次安装或从旧版未授权状态升级时默认启用并建立受托授权，用户之后仍可关闭包、贡献点或收窄授权。手动同步间隔默认且最少为 5 分钟，可配置到 24 小时；自动计划最短 1 小时，建议周期 24 小时。
...
3. 常驻缓存只保存 2 分钟，按账号 Session、官方插件版本、贡献点、捕获规则和参数隔离。身份响应在主进程先投影为 handle、稳定 ID、昵称、头像、简介和必要计数；请求头、Cookie、Authorization、查询变量、完整 URL 与响应中的其他字段不会进入缓存或 QuickJS。
