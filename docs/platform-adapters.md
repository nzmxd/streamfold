...
- 同步前后复验稳定身份；账号切换、游标停滞、重复冲突或结构异常时整次事务不提交。达到适配器声明的有界监听上限时按平台策略记录警告。
...
插件 ID：`streamfold.x`，版本 1.2.0；随应用签名分发，在 QuickJS Utility Process 中运行，最低要求归页 0.7.11。首次安装或从旧版未授权状态升级时默认启用并建立受托授权，用户之后仍可关闭包、贡献点或收窄授权。身份响应由清单声明的通用后台 supervisor 长驻监听，连接中断后自动退避重连；手动同步间隔默认且最少为 5 分钟，可配置到 24 小时，自动计划最短 1 小时，建议周期 24 小时。
...
1. 未绑定账号打开 X 官方页面后，宿主在该账号工作区中建立常驻响应监听；监听本身没有“等待首响应”的整体超时，插件调用只登记清单规则、读取现有缓存并立即返回“后台监听中”。
2. 捕获 `account/settings` 后自动按当前 handle 登记资料页监听；完整 `UserByScreenName` 到达后才生成一次身份预览并提示绑定，不轮询，也不把尚未到达的响应记为错误。
3. 常驻缓存只保存 2 分钟，按账号 Session、官方插件版本、贡献点、捕获规则、字段白名单和参数隔离。响应 JSON 只投影清单列出的公开身份标量，再执行凭证脱敏后缓存；请求头、请求正文、查询变量、实际请求 URL 和所有未声明字段都不会进入缓存或 QuickJS。
4. 用户确认和同步前后不复用发现缓存，而是重新执行一次有界的“登录设置 handle → 当前资料 → 稳定 `rest_id`”鲜活核验，并由宿主与预期稳定 ID 比对；预期 ID 不传入 X 插件，若实际登录账号已切换，宿主会把账号标记为身份不一致。
5. 退出登录、切换账号或 handle/ID 不一致时整次事务不提交。额外的 `UserByRestId` 反查不与当前会话绑定，而且 X 页面可能不再发起该请求，因此不再将它作为身份成立条件。
...
`recent_20` 最多捕获 5 个时间线响应，`recent_100` 最多捕获 20 个；每个响应当前包含约 20 个时间线条目。适配器按 Bottom cursor 检查推进、去重并过滤不属于本人作品的条目，达到授权目标数后截断。账号本身内容少且时间线明确终止时允许返回较少结果；有界监听因响应数、时间或总量限制停止后仍有 Bottom cursor 时，提交已经通过本人身份和结构校验的内容，并记录实际响应数、声明上限、已验证内容数和请求范围。重复游标、冲突重复、账号不一致或结构异常仍会使整次同步失败。
...
自动化夹具覆盖当前 `core`/`avatar`/`profile_bio` 资料结构、旧版 legacy 回退、长帖、图片/视频、引用、回复与纯转帖过滤、64 位以上字符串 ID、分页终止、监听上限降级警告和异常分页回滚。发布前仍需使用本人测试账号在应用隔离 Session 中完成 20/100 条在线验收，且不得保存真实 Cookie、请求头或完整响应。
