...
插件 ID：`streamfold.x`，版本 1.1.1；随应用签名分发，在 QuickJS Utility Process 中运行。首次安装或从旧版未授权状态升级时默认启用并建立受托授权，用户之后仍可关闭包、贡献点或收窄授权。手动同步间隔默认且最少为 5 分钟，可配置到 24 小时；自动计划最短 1 小时，建议周期 24 小时。
...
1. 每次身份读取先捕获 `account/settings`；宿主会等待并发正文读取完成、严格遵守响应数量上限，缺少该响应时不会把公开资料页误判为有效登录。
