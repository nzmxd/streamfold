...
function validateBackgroundCapture(
  ...
  let identityDiscovery: PlatformBackgroundCaptureDeclaration['identityDiscovery']
...
function backgroundResponseFieldPath(value: string): string {
  ...
  if (segments.at(-1) === '*' ||
      ...
      segments.some((segment) => sensitiveBackgroundField(segment))) {
    throw new Error('后台响应字段路径非法或包含敏感字段')
  }
  return `/${segments.map((segment) => segment.replace(/~/gu, '~0').replace(/\//gu, '~1')).join('/')}`
}

function sensitiveBackgroundField(value: string): boolean {
