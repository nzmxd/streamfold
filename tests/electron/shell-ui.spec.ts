...
test('首次启动使用海蓝默认主题色', async () => {
  await expect(page.locator('html')).toHaveAttribute('data-theme-color', '2563eb')
...
test('分析支持发帖快捷范围、发布周对比、周统计和内容趋势窗口', async ({}, testInfo) => {
  ...
  await expect.poll(() => analysisPage.evaluate((element) => element.scrollTop)).toBe(0)
...
test('日志中心持久记录渲染错误和 IPC 校验错误', async () => {
  await page.evaluate(async () => {
    const error = new Error('E2E_RENDER_DIAGNOSTIC token=E2E_RENDER_TOKEN')
  ...
  await expect(page.locator('.log-stack h3')).toHaveText('错误详情')
  await expect(page.locator('.log-stack pre')).toContainText('E2E_RENDER_DIAGNOSTIC')
  await expect(page.locator('.log-detail')).not.toContainText('E2E_RENDER_TOKEN')
